...
public class MySqlOradorRepository implements OradorRepository {
	...
	public void save(Orador orador) {
		...
		//El intento de conexion lo hago directamente en el try
		try(Connection con = AdministradorDeConexiones.getConnection()){
			...
			statement.setDate(5,new java.sql.Date(DateUtils.asDate(orador.getFechaAlta()).getTime())); //Pasar de date a java.sql.Date
			
			statement.executeUpdate(); //comando para realizar las acciones INSERT/UPDATE o DELETE
			
			ResultSet res = statement.getGeneratedKeys();
			if(res.next()) {
				Long id = res.getLong(1); //Aca se halla el ID.
				orador.setId(id);
			}
		} catch(Exception e) {
	...
	public List<Orador> findAll() {
		...
		String sql = "select id_orador, nombre, apellido, mail, tema, fecha_alta from orador"; //sin WHERE porque seleccionamos todo.
		
		List<Orador> oradores = new ArrayList<>(); //Buscar, se ve recien con spring
